...
export async function searchSeries(
  ...
  if (type && type !== 'all' && type !== 'Todo')
    conditions.push(sql`${series.type} LIKE ${`%${type}%`}`);
...
export async function getFilterMetadata(db: DrizzleD1Database<typeof schema>) {
  ...
  const [authorsData, artistsData, publishersData, magazinesData] = await Promise.all([
    db
      .selectDistinct({ value: series.author })
      .from(series)
      .where(sql`${series.author} IS NOT NULL AND ${series.author} != ''`)
      .all(),
    db
      .selectDistinct({ value: series.artist })
      .from(series)
      .where(sql`${series.artist} IS NOT NULL AND ${series.artist} != ''`)
      .all(),
    db
      .selectDistinct({ value: series.publishedBy })
      .from(series)
      .where(sql`${series.publishedBy} IS NOT NULL AND ${series.publishedBy} != ''`)
      .all(),
    db
      .selectDistinct({ value: series.serializedBy })
      .from(series)
      .where(sql`${series.serializedBy} IS NOT NULL AND ${series.serializedBy} != ''`)
      .all(),
  ]);
