export async function hashIpAddress(ip: string): Promise<string> {
  ...
  if (salt === 'development-only-salt' && import.meta.env.PROD) {
    console.warn(
      '[Security] Utilizando salt de desarrollo en producción. ¡Configura INTERNAL_CRYPTO_SALT!'
    );
  }
...
export async function verifySignature(
  ...
  try {
    ...
    return await crypto.subtle.verify('HMAC', key, signatureBytes, encoder.encode(dataToVerify));
  } catch {
    return false;
  }
}
    ...
    const signedPages = await Promise.all(
      manifest.pages.map(async (page: any, idx: number) => {
        ...
        if (urlToSign) {
          ...
          console.warn(
            `[CRYPTO] signManifest: Page at index ${idx} is missing url/imageUrl. Structure: ${JSON.stringify(page)}`
          );
        }
        return signedPage;
      })
