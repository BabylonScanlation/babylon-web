...
export const POST: APIRoute = async ({
  ...
  try {
    ...
    if (!commentId) {
      if (contentType.includes('application/json')) return new Response(JSON.stringify({ error: 'ID requerido' }), { status: 400 });
    ...
    const existing = await drizzleDb.select().from(comments).where(eq(comments.id, parseInt(commentId))).get();
    
    if (!existing) {
         if (contentType.includes('application/json')) return new Response(JSON.stringify({ error: 'No encontrado' }), { status: 404 });
    ...
    if (!isOwner && !isAdmin) {
        if (contentType.includes('application/json')) return new Response(JSON.stringify({ error: 'Prohibido' }), { status: 403 });
