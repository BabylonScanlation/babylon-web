...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!seriesId || !targetTotal) {
      return new Response(
        JSON.stringify({ error: 'Datos inválidos. Faltan campos obligatorios.' }),
        { status: 400 }
      );
    }
    ...
    if (!seriesData || !seriesData.isAppSeries) {
      return new Response(
        JSON.stringify({ error: 'Esta serie no está configurada como "Solo App".' }),
        { status: 400 }
      );
    }
    ...
    if (currentMax >= targetTotal) {
      return new Response(
        JSON.stringify({ message: 'La serie ya tiene esa cantidad o más de capítulos.' }),
        { status: 200 }
      );
    }
