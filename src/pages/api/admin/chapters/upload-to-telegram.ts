...
export const POST = createApiRoute({ auth: 'admin' }, async ({ request, locals }) => {
  ...
  try {
    ...
    if (!seriesId || !zipFile || !(zipFile instanceof File)) {
      return new Response(
        JSON.stringify({ error: 'Faltan datos obligatorios (Serie o Archivo ZIP).' }),
        { status: 400 }
      );
    }
    ...
    if (!seriesData || !seriesData.topicId) {
      return new Response(
        JSON.stringify({ error: 'La serie no existe o no tiene un Topic de Telegram asignado.' }),
        { status: 404 }
      );
    }
    ...
    try {
      const existing = await db
        ...
        .from(chapters)
        .where(
          and(eq(chapters.seriesId, parseInt(seriesId)), eq(chapters.chapterNumber, chapterNumber))
        )
        .get();
