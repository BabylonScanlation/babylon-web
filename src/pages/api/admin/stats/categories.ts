...
export const GET = createApiRoute({ auth: 'admin' }, async ({ locals, request }) => {
  ...
  try {
    ...
    if (days) demoQuery = demoQuery.where(sql`${chapterViews.viewedAt} >= datetime('now', '-' || ${days} || ' days')`) as any;
    ...
    if (days) genreRawQuery = genreRawQuery.where(sql`${chapterViews.viewedAt} >= datetime('now', '-' || ${days} || ' days')`) as any;
    ...
    return new Response(JSON.stringify({ error: 'No se pudieron obtener las estadísticas por categoría.' }), { status: 500 });
  }
});
