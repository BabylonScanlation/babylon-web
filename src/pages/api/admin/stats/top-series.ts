  ...
  try {
    let query = db
      .select({
        ...
        avgRating:
          sql<number>`(SELECT AVG(rating) FROM SeriesRatings WHERE series_id = ${series.id})`.as(
            'avgRating'
          ),
        voteCount:
          sql<number>`(SELECT COUNT(*) FROM SeriesRatings WHERE series_id = ${series.id})`.as(
            'voteCount'
          ),
      })
      .from(series)
    ...
    return new Response(
      JSON.stringify({ error: 'No se pudieron obtener las series más populares.' }),
      { status: 500 }
    );
  }
});
