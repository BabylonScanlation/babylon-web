...
import { chapterViews, seriesViews, sessions, anonymousUsers } from '../../../db/schema';
...
export const POST = createApiRoute({ auth: 'admin' }, async ({ locals }) => {
  ...
  try {
    ...
    const cvDeleted = await db.delete(chapterViews).where(lt(chapterViews.viewedAt, thirtyDaysAgo)).run();
    const svDeleted = await db.delete(seriesViews).where(lt(seriesViews.viewedAt, thirtyDaysAgo)).run();

    // 2. Limpiar Sesiones expiradas
    const sessionsDeleted = await db.delete(sessions).where(lt(sessions.expiresAt, sessionThreshold)).run();

    // 3. Limpiar Usuarios anónimos inactivos (más de 7 días)
    const anonDeleted = await db.delete(anonymousUsers).where(lt(anonymousUsers.updatedAt, sevenDaysAgo)).run();
