...
export const POST = createApiRoute({ auth: 'admin' }, async ({ request, locals }) => {
  ...
  if (method === 'DELETE') {
    ...
    if (uid === superAdminUid) {
      return new Response(
        JSON.stringify({ error: 'No se puede eliminar al Super Administrador' }),
        { status: 400 }
      );
    }
    ...
    return new Response(JSON.stringify({ success: 'Administrador eliminado con éxito' }), {
      status: 200,
    });
  } else {
    if (!uid)
    ...
    return new Response(JSON.stringify({ success: 'Administrador añadido con éxito' }), {
      status: 200,
    });
  }
});
