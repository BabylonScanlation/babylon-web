...
import { createNews, getAllNews } from '../../../../lib/db';

export const GET = createApiRoute({ auth: 'admin' }, async (context) => {
...
export const POST = createApiRoute({ auth: 'admin' }, async (context) => {
  ...
  if (!title || !content) {
    return new Response(
      JSON.stringify({ error: 'Faltan campos obligatorios (título o contenido).' }),
      { status: 400 }
    );
  }
