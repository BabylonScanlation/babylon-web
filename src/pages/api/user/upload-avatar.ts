...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (file.size > 2 * 1024 * 1024) { // 2MB Limit
      return new Response(JSON.stringify({ error: 'La imagen es demasiado pesada (Máx 2MB).' }), { status: 400 });
    }
    ...
    const currentUser = await db.select({ avatarUrl: users.avatarUrl }).from(users).where(eq(users.id, user.uid)).get();
    ...
    return new Response(JSON.stringify({ error: 'Error interno al subir la imagen.' }), { status: 500 });
  }
};