...
export const POST: APIRoute = async ({ request, cookies, locals }) => {
  ...
  try {
    ...
    if (!validation.success) {
      return new Response(
        JSON.stringify({
          error: 'El token de autenticación no fue proporcionado o tiene un formato inválido.',
        }),
        { status: 400 }
      );
    }
    ...
    if (!decodedToken?.sub) {
      return new Response(
        JSON.stringify({ error: 'Token inválido: UID no encontrado o formato incorrecto.' }),
        { status: 401 }
      );
    }
    ...
    return new Response(JSON.stringify({ error: 'Falló la autenticación del usuario.' }), {
      status: 401,
    });
  }
};
