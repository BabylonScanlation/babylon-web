...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    console.log(
      `>>> [DEBUG] Password received (API): ${password ? 'YES' : 'NO'}`
    );
    console.log(
      `>>> [DEBUG] Admin password from env (API): ${adminPassword ? 'DEFINED' : 'UNDEFINED'}`
    );
    ...
    if (password === adminPassword) {
      ...
      console.log(
        '>>> [DEBUG] API Incorrect password, redirecting to /admin?error=Contraseña incorrecta.'
      );
    ...
    return new Response(
      `Internal Server Error en API Login: ${e instanceof Error ? e.message : String(e)}`,
      { status: 500 }
    );
  }
};
