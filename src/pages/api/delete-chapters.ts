...
export const POST = createApiRoute(
  ...
  async (context: APIContext) => {
    ...
    if (chapterIds.length === 0 || chapterIds.some(isNaN)) {
      return new Response(JSON.stringify({ error: 'No se proporcionaron IDs de capítulo válidos.' }), { 
        status: 400, 
        headers: { 'Content-Type': 'application/json' } 
      });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ 
        success: true, 
        message: chapterIds.length > 1 ? `${chapterIds.length} capítulos eliminados` : 'Capítulo eliminado con éxito' 
      }), { 
        status: 200, 
        headers: { 'Content-Type': 'application/json' } 
      });
      ...
      return new Response(JSON.stringify({ error: 'Error interno al intentar eliminar: ' + e.message }), { 
        status: 500, 
        headers: { 'Content-Type': 'application/json' } 
      });
    }
